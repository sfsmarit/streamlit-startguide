...
st.markdown(
    ...
    ただし、ウェブアプリは万能なツールではありません。業務内容に応じて、適切な手段を選択することが重要です。
    """
)
...
st.markdown(
    ...
    一方で、大量のローカルデータを処理する用途には適していません。こうした場合は、デスクトップアプリや専用ツールなどを検討するとよいでしょう。
    """
)
...
st.markdown(
    ...
    【まとめ】  
    簡単にアプリ化できると思ったら自作すればよいです。一方で、技術的・職務的に困難な場合は IT チームに依頼すべきです。
    """
)
...
st.markdown(
    ...
    - Python では`sqlalchemy`ライブラリを使うことで、SQL なしでどの DB にも接続できる
    """
)
...
st.markdown(
    ...
    しかし、Streamlit の拡張性やパフォーマンスの制約から、他の一般的な Web 開発手法が有利になる場合が多いです。
    """
)
